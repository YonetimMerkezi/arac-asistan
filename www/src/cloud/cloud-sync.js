...
export async function collectLocalSnapshot(){
  const db=getDb();
  const tables={};
  for(const table of TABLES){try{const result=await db.query(`SELECT * FROM ${table}`);tables[table]=result.values??[];}catch{tables[table]=[];}}
  const {keys}=await Preferences.keys();
  const preferences={};
  for(const key of keys){if(key.startsWith('sda_firebase_'))continue;const {value}=await Preferences.get({key});preferences[key]=value;}
...
export async function getRemoteSnapshot(){
  ...
  if(!payload||!payload.tables){logWarn('cloud-sync','Bulut yedeği beklenen biçimde değil.');return null;}
  return {payload,clientUpdatedAt:remote.clientUpdatedAt??null,schemaVersion:remote.schemaVersion??payload.formatVersion??1};
}

export function getLastSyncInfo(){try{return JSON.parse(localStorage.getItem(LOCAL_META_KEY)||'null');}catch{return null;}}
