...
import { getDrivingRoute, getCachedDrivingRoute } from '../maps/route-service.js';
...
export async function drawRouteTo(map, destination, container, origin = null) {
  ...
  let routes = await getDrivingRoute(
    { lat: current.latitude ?? current.lat, lon: current.longitude ?? current.lon },
    { lat: destination.lat, lon: destination.lon },
    { destinationLabel: destination.label ?? destination.searchQuery ?? '', cache: true },
  );

  let offlineFallback = false;
  if (!routes || routes.length === 0) {
    routes = await getCachedDrivingRoute(destination);
    offlineFallback = Boolean(routes?.length);
  }
 ...
 * @param {HTMLElement} container
 */
function replacePrimaryRoute(map, updatedRoute, destination, container) {
  if (routeLine) map.removeLayer(routeLine);
  routeLine = L.polyline(updatedRoute.coordinates, { color: '#4FD8E0', weight: 5 }).addTo(map);
  map.fitBounds(routeLine.getBounds(), { padding: [24, 24] });

  const summaryEl = container.querySelector('[data-route-summary]');
  if (summaryEl) {
    setField(summaryEl, 'route-distance', `${updatedRoute.distanceKm.toFixed(1)} km`);
    setField(summaryEl, 'route-duration', `${Math.max(1, Math.round(updatedRoute.durationMinutes))} dk`);
    const eta = new Date(Date.now() + updatedRoute.durationMinutes * 60000);
...
function selectRoute(map, routes, selectedIndex, destination, container) {
  ...
  if (statusEl) statusEl.textContent = offlineFallback ? 'Çevrimdışı kayıtlı rota kullanılıyor.' : '';
  if (summaryEl) {
  ...
  startGuidance(selected, destination, {
    onRouteUpdate: (updatedRoute) => {
      replacePrimaryRoute(map, updatedRoute, destination, container);
    },
  });
